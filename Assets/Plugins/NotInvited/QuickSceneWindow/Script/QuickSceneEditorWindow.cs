...
namespace NotInvited.QuickSceneWindow
{
    public class QuickSceneEditorWindow : EditorWindow
    {
        ...
        private void DrawAllSceneButtons()
        {
            ...
            for (int i = 0; i < sceneNames.Count; i++)
            {
                if (IsSceneButtonShow(sceneNames[i]))
                {
                    ...
                    if (selected)
                    {
                        ColorUtility.TryParseHtmlString("#3E5F96", out Color selectedColor);
                        style.normal.background = Texture2DUtils.GetColorTexture(selectedColor);
                    }
                    ...
                    {
                        GUILayout.Button("Enter to load", EditorStyles.toolbarButton, GUILayout.MaxWidth(78f));
                    }
                    else if (GUILayout.Button(new GUIContent(EditorGUIUtility.FindTexture("d_Folder Icon")), GUILayout.Width(20f), GUILayout.Height(20f)))
                    {
                        SelectSceneFolder(i);
                    }
        ...
        private void DrawOpenSceneBar()
        {
            ...
            searchName = GUILayout.TextField(searchName, GUI.skin.FindStyle("ToolbarSearchTextField"), GUILayout.MaxWidth(200f));
#else
        ...
        private void SelectSceneFolder(int index)
        {
            EditorUtility.FocusProjectWindow();
            var obj = AssetDatabase.LoadAssetAtPath(scenePaths[index], typeof(UnityEngine.Object));
